...
class reservation : AppCompatActivity(),DatePickerDialog.OnDateSetListener,TimePickerDialog.OnTimeSetListener,PaymentResultListener {
    var day = 0
    var month = 0
    var year = 0
    var hour = 0
           ...
           val apiuser = RetrofitApi.create().addReservation(reservation)
            ...
            apiuser.enqueue(object: Callback<Reservation> {
                ...
                override fun onFailure(call: Call<Reservation>, t: Throwable) {
                    ...
                    checkout.setKeyID("rzp_test_k8OdVhiGrKJNcB")
                    val `object` = JSONObject()
                    try {
                        `object`.put("name", "CaryPark")
                        `object`.put("description", "Payment")
                        `object`.put("currency", "USD")
                        `object`.put("amount", prix.toInt()*100)
                        `object`.put("prefill.contact", "+21626195155")
                        `object`.put("prefill.email", "yesmine.khayati@esprit.tn")
                        checkout.open(this@reservation, `object`)
